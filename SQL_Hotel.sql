...
CREATE TABLE NguoiDung (
    MaNguoiDung NVARCHAR(20) PRIMARY KEY,              -- Mã định danh người dùng với tiền tố 'ND'
    Vaitro NVARCHAR(20) NOT NULL DEFAULT 'KhachHang',  -- Vai trò mặc định là Khách hàng
    ...
    CanCuocCongDan NVARCHAR(20) UNIQUE NULL,          -- Số CCCD
    SoDienThoai NVARCHAR(15) UNIQUE NULL,             -- Số điện thoại
    ...
    LoaiDangNhap NVARCHAR(20) NOT NULL DEFAULT 'ThuCong', -- Loại đăng nhập (ThuCong, Google, Facebook)
    NgayTao DATETIME DEFAULT GETDATE()                -- Ngày tạo
);

-- Bảng Phong
CREATE TABLE Phong (
    MaPhong NVARCHAR(20) PRIMARY KEY,                 -- Mã phòng với tiền tố 'P'
    ...
    UrlAnhChinh NVARCHAR(255) NOT NULL , -- URL ảnh mặc định
    MotaPhong TEXT NULL,                              -- Mô tả phòng
...
CREATE TABLE PhongAnh (
    MaAnh INT IDENTITY(1,1) PRIMARY KEY,              -- Mã ảnh
    MaPhong NVARCHAR(20),                             -- Mã phòng liên kết với tiền tố 'P'
...
CREATE TABLE TienNghi (
    MaTienNghi NVARCHAR(20) PRIMARY KEY,              -- Mã tiện nghi với tiền tố 'TN'
...
CREATE TABLE Phong_TienNghi (
    MaPhong NVARCHAR(20) NOT NULL,                    -- Mã phòng
    MaTienNghi NVARCHAR(20) NOT NULL,                 -- Mã tiện nghi
...
CREATE TABLE GiamGia (
    MaGiamGia NVARCHAR(20) PRIMARY KEY,               -- Mã giảm giá với tiền tố 'GG'
    TenGiamGia NVARCHAR(50) NOT NULL,                 -- Tên chương trình giảm giá
    LoaiGiamGia NVARCHAR(20) CHECK (LoaiGiamGia IN ('PhanTram', 'SoTien')), -- Loại giảm giá
...
CREATE TABLE Phong_GiamGia (
    ...
    MaGiamGia NVARCHAR(20),                           -- Mã giảm giá
...
-- Bảng DichVu
CREATE TABLE DichVu (
    MaDichVu NVARCHAR(20) PRIMARY KEY,                -- Mã dịch vụ với tiền tố 'DV'
    ...
    HinhAnhDichVu NVARCHAR(MAX) NOT NULL, -- URL ảnh mặc định
    SoLuong INT NOT NULL DEFAULT 0,                   -- Số lượng dịch vụ
    TrangThai INT NOT NULL DEFAULT 1,                 -- Trạng thái dịch vụ
...
CREATE TABLE DatPhong (
    MaDatPhong NVARCHAR(20) PRIMARY KEY,              -- Mã đặt phòng với tiền tố 'DP'
    MaNguoiDung NVARCHAR(20) NOT NULL,                -- Mã người dùng
    MaPhong NVARCHAR(20) NOT NULL,                    -- Mã phòng
    NgayDat DATETIME DEFAULT GETDATE(),               -- Ngày đặt
    NgayCheckIn DATETIME,                             -- Ngày check-in
    NgayCheckOut DATETIME,                            -- Ngày check-out
    TinhTrangDatPhong NVARCHAR(1) CHECK (TinhTrangDatPhong IN ('1', '2', '3')) NOT NULL, -- 1: Đã xác nhận, 2: Đang chờ, 3: Hủy
...
CREATE TABLE DatDichVu (
    MaDatDichVu NVARCHAR(20) PRIMARY KEY,             -- Mã đặt dịch vụ với tiền tố 'DDV'
    MaDatPhong NVARCHAR(20) NOT NULL,                 -- Mã đặt phòng
    MaDichVu NVARCHAR(20) NOT NULL,                   -- Mã dịch vụ
    SoLuong INT NOT NULL DEFAULT 1,                   -- Số lượng dịch vụ
    MaHoaDon NVARCHAR(20) NULL,                       -- Mã hóa đơn liên kết
...
-- Bảng HoaDon
CREATE TABLE HoaDon (
    MaHoaDon NVARCHAR(20) PRIMARY KEY,                -- Mã hóa đơn với tiền tố 'HD'
    MaNguoiDung NVARCHAR(20) NOT NULL,                -- Mã người dùng
    MaDatPhong NVARCHAR(20) NOT NULL,                 -- Mã đặt phòng
    ...
    TinhTrangHoaDon NVARCHAR(1) CHECK (TinhTrangHoaDon IN ('1', '2')) NOT NULL, -- 1: Đã thanh toán, 2: Chưa thanh toán
    FOREIGN KEY (MaNguoiDung) REFERENCES NguoiDung(MaNguoiDung),
    FOREIGN KEY (MaDatPhong) REFERENCES DatPhong(MaDatPhong)
);
...
CREATE TABLE ChiTietHoaDon (
    MaChiTiet NVARCHAR(20) PRIMARY KEY,               -- Mã chi tiết hóa đơn
    MaHoaDon NVARCHAR(20) NOT NULL,                  -- Mã hóa đơn
    LoaiKhoanMuc NVARCHAR(20) NOT NULL,              -- Loại khoản mục (Phong, DichVu, GiamGia)
    MaKhoanMuc NVARCHAR(20) NOT NULL,                -- Mã khoản mục (MaPhong, MaDichVu, MaGiamGia)
...
CREATE TABLE ThanhToan (
    MaThanhToan NVARCHAR(20) PRIMARY KEY,             -- Mã thanh toán với tiền tố 'TT'
    MaHoaDon NVARCHAR(20) NOT NULL,                   -- Mã hóa đơn
...
CREATE TABLE Feedback (
    MaFeedback NVARCHAR(20) PRIMARY KEY,              -- Mã phản hồi với tiền tố 'FB'
    MaPhong NVARCHAR(20) NOT NULL,                    -- Mã phòng
    MaNguoiDung NVARCHAR(20) NOT NULL,                -- Mã người dùng
    SoSao INT CHECK (SoSao BETWEEN 1 AND 5) NOT NULL, -- Số sao (1-5 sao)
    BinhLuan NVARCHAR(1000) NULL,                     -- Bình luận của người dùng
...
CREATE TABLE NhanVien (
    MaNhanVien NVARCHAR(20) PRIMARY KEY,             -- Mã nhân viên
    MaNguoiDung NVARCHAR(20) NOT NULL,               -- Mã người dùng
    ...
    TrangThai NVARCHAR(20) DEFAULT 'HoatDong',       -- Trạng thái (Hoạt động, Nghỉ việc)
    FOREIGN KEY (MaNguoiDung) REFERENCES NguoiDung(MaNguoiDung)
);

-- Bảng LichSuGiaoDich
CREATE TABLE LichSuGiaoDich (
    MaGiaoDich NVARCHAR(20) PRIMARY KEY,             -- Mã giao dịch
    MaNguoiDung NVARCHAR(20) NOT NULL,               -- Mã người dùng
...
-- Bảng BaoCao
CREATE TABLE BaoCao (
    MaBaoCao NVARCHAR(20) PRIMARY KEY,               -- Mã báo cáo
...
CREATE TABLE ChiTietBaoCao (
    MaChiTiet NVARCHAR(20) PRIMARY KEY,               -- Mã chi tiết báo cáo
    MaBaoCao NVARCHAR(20) NOT NULL,                  -- Mã báo cáo
...
-- Bảng LichSuTinhTrangPhong
CREATE TABLE LichSuTinhTrangPhong (
    MaLichSu NVARCHAR(20) PRIMARY KEY,               -- Mã lịch sử
    MaPhong NVARCHAR(20) NOT NULL,                   -- Mã phòng
    TinhTrangCu NVARCHAR(1) NOT NULL,                -- Trạng thái cũ
    TinhTrangMoi NVARCHAR(1) NOT NULL,               -- Trạng thái mới
